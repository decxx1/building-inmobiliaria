...
<template>
    <article class="cursor-pointer w-64 min-[310px]:w-72 sm:w-80 h-max bg-white border border-primary-web-700 shadow hover:shadow-primary-web-700 hover:scale-105 transition-all duration-300 dark:bg-gray-700 dark:border-gray-700">
        <Link :href="props.admin ? '' : route('inmuebles.show', {id: props.id})">
            <header>
                <figure class="relative">
                    <span class="w-full h-56 block">
                        <img class="w-full h-full object-cover" :src="props.imageCover ? props.imageCover : baseUrl + 'images/properties/sin-propiedad.svg'" alt="imagen de la propiedad" />
            ...
            <div class="bg-primary-web-700 dark:bg-gray-800 py-2">
                <div v-if="!props.priceHidden && props.price">
                    <p :class="['text-white text-right font-extrabold px-3', sizeLetterClass(props.price)]">{{ props.isPriceDollar ? 'US$' : 'ARS$' }} {{ numberFormat(props.price) }}</p>
                </div>
                <div v-else>
            ...
            <div class="p-2 text-left">
                <h5 class="mb-2 text-xl font-bold tracking-tight text-primary-web-700 dark:text-white">{{ props.address }}</h5>
                <p class="mb-3 font-normal text-gray-700 dark:text-gray-300 inline" v-html="formattedDescription(props.description)"></p>
                <span v-if="props.description.length > 100"> (...)</span>
            </div>
            <footer class="bg-primary-web-700 dark:bg-gray-800 py-2">
                <div class="px-2 flex flex-wrap flex-row justify-start text-white">
                    <div v-if="props.superficieTotal" class="flex flex-row items-center mr-3">
                    ...
                    <div v-if="props.dormitorios && props.dormitorios !== 'Indistinto'" class="flex flex-row items-center mr-3">
                        <img class="w-6 mr-1" src="/images/icons-property/dormitorio2.svg" alt="habitaciones que tiene la propiedad" />
                        <span>{{ props.dormitorios }}</span>
                    </div>
                    <div v-if="props.cochera && props.cochera !== 'Indistinto'" class="flex flex-row items-center mr-3">
                        <img class="w-6 mr-1" src="/images/icons-property/cochera.svg" alt="tiene cochera el inmueble" />
                        <span>{{ props.cochera === 'Sin cochera' ? 'No' : 'Si' }}</span>
                    </div>
                </div>
            </footer>
        </Link>
